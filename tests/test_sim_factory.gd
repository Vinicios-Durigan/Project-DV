...
func test_monta_os_quatro_sistemas_na_ordem_fixa() -> void:
	var systems := _world.get_systems()
	assert_eq(systems.size(), 4, "os 4 sistemas do slice")
	assert_true(_sistema(0) is InventorySystem, "1. Inventory valida e cobra")
	assert_true(_sistema(1) is ShippingSystem, "2. Shipping vende antes de o dia virar")
	assert_true(_sistema(2) is FarmSystem, "3. Farm cresce depois da venda")
	assert_true(_sistema(3) is TimeSystem, "4. Time vira o calendário por último")

func test_registra_todo_state_no_save_na_ordem_do_formato() -> void:
	assert_eq(_world.state_keys(), ["time", "inventory", "farm", "shipping"],
		"as chaves são o formato do save — GAMEPLAY §10")
...
func test_o_state_registrado_e_o_mesmo_que_o_sistema_usa() -> void:
	...
	assert_same(_world.get_state("inventory"), (_sistema(0) as InventorySystem).get_state())
	assert_same(_world.get_state("shipping"), (_sistema(1) as ShippingSystem).get_state())
	assert_same(_world.get_state("farm"), (_sistema(2) as FarmSystem).get_state())

func test_carrega_os_catalogos_de_data() -> void:
	assert_eq(_factory.get_item_catalog().size(), 8, "os 8 itens de data/items/")
	assert_eq(_factory.get_crop_catalog().ids(), ["abobora", "cenoura", "morango", "rabanete"],
		"as 4 culturas de data/crops/")

func test_os_sistemas_compartilham_o_mesmo_catalogo() -> void:
	assert_same((_sistema(0) as InventorySystem).get_catalog(), _factory.get_item_catalog(),
		"catálogo é leitura livre, mas é um só")
	assert_same((_sistema(1) as ShippingSystem).get_catalog(), _factory.get_item_catalog())
	assert_same((_sistema(2) as FarmSystem).get_catalog(), _factory.get_crop_catalog())
	assert_same((_sistema(0) as InventorySystem).get_crop_catalog(), _factory.get_crop_catalog())

func test_mundo_novo_comeca_como_o_gameplay_manda() -> void:
	var inv := (_sistema(0) as InventorySystem).get_state().get_player(SimFactory.PLAYER_PADRAO)
...
func test_o_mundo_montado_joga_de_verdade() -> void:
	...
	assert_true((_sistema(2) as FarmSystem).get_state().peek_plot(0, 0).tem_cultura(),
